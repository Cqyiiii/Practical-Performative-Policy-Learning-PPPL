...
# generating feature manipulation when coarser discretization is considered
...
def StrategicGD_coarse(seed, dim_v, card_u, beta, H, len_seq, num_epoch, T, n, n_eval, lr_pi = 0.01, lr_prob = 0.01, cost_coef = 0.05):
    print(seed)
    ...
    optimizer_pi = optim.AdamW(model_pi.parameters(), lr=lr_pi)     
    ...
    plt.savefig("coarse_curves_{}/card{}_true_card_{}_seed{}.pdf".format(true_card_u, card_u, true_card_u, seed))
...
# fix test set for policy evaluation
u_eval, v_eval = x_generate(n_eval, dim_v, trans, seed=0) # fix eval
data_init_seed = 0
seed_list = range(data_init_seed, data_init_seed + 10)
dataset = {}
for s in seed_list:
...
for card_u in [10, 5, 4, 3, 2]:
    ...
    spg_cv = []
    init_seed = data_init_seed
    for rep_index in range(10):
        spg_curve = StrategicGD_coarse(cost_coef=cost_coef, lr_pi=0.05, lr_prob=0.1, seed=init_seed + rep_index, dim_v=dim_v, card_u=card_u, beta=beta, H=H, len_seq=len_seq, num_epoch=num_epoch, T=T, n=n, n_eval=n_eval)                
        coarse_spg_cv_list.append(spg_curve)    
        
    torch.save(coarse_spg_cv_list, "coarse_curves_{}/coarse_spg_cv_list_true_2-10card{}_new.pkl".format(true_card_u, true_card_u))        